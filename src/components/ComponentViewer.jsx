...
// ── Zoom thresholds ──────────────────────────────────────────────────────────
const MIN_DIST_ZOOM = 0.8   // Absolute minimum zoom distance
...
// ── ComponentModel ────────────────────────────────────────────────────────────
function ComponentModel({ modelPath, scrollProgress = 0, explodeTrigger = 'scroll', onInitZoomLimit }) {
  // Clone the raw cached scene so shared state across renders is avoided.
  const { scene: rawScene } = useGLTF(modelPath, resolveAsset('/draco/'))
  ...
  useFrame(state => {
    ...
    if (!initialized.current && scene) {
      ...
      return // wait one frame for Bounds to finish moving the camera
    }

    const orbitTarget = controls?.target ?? new THREE.Vector3()
    const currentDist = state.camera.position.distanceTo(orbitTarget)

    // Capture the camera distance AFTER Bounds has fitted the object
    if (initialDistRef.current === null) {
      initialDistRef.current = currentDist
      // Tell parent to restrict zoom so user can't zoom past fully exploded state
      if (explodeTrigger === 'zoom' && onInitZoomLimit) {
        onInitZoomLimit(currentDist * 0.35)
      }
    }
    ...
    if (explodeTrigger === 'zoom') {
      // Start exploding when user zooms in 15% from initial fit distance
      const explodeStart = initialDistRef.current * 0.85
      // Fully exploded when zoomed in 65% from initial fit distance
      const explodeFull = initialDistRef.current * 0.35

      if (currentDist < explodeStart) {
        target = (explodeStart - currentDist) / (explodeStart - explodeFull)
    ...
    const explodeScale = modelRadius * 1.5
...
export default function ComponentViewer({
  ...
  const [minZoomDist, setMinZoomDist] = useState(isZoomTrigger ? MIN_DIST_ZOOM : 1.8)

  const modelPath = resolveAsset(`/models/${modelFile}`)
              ...
              <Suspense fallback={<Center><FallbackBox color={color} /></Center>}>
                <Center>
                  <ComponentModel
                    ...
                    onInitZoomLimit={setMinZoomDist}
                  />
                </Center>
              </Suspense>
            </ErrorBoundary>
